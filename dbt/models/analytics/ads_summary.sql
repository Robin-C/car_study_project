...
summary as (
    select distinct ad_id
	 ...
	 , first_value(published_at) over(partition by ad_id order by scrape_rank desc) as published_at 
...
from dbt_stg.stg_ads_scrape_rank
group by ad_id, url, model, trim, scrape_rank, price, seller, registered_on, km, transmission, engine, hp, color, efficiency, co2, site_rec, region, published_at, started_scrape_at
),

set_status as (
    select *
         , case when max(last_scrape_rank) - last_scrape_rank > 4 then 'SOLD' else 'ONGOING' end as status -- if we havent seen a car in ~2 days then we assume it's been sold
    from summary
